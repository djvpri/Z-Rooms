...
export default function QRApprovePage() {
  ...
  async function handleFaceLogin() {
    setShowCamera(true)
    // Face capture will be handled by camera component
  }
  ...
  async function onFaceCapture(imageData: string) {
    ...
    try {
      ...
      if (!faceRes.ok) {
        setError(faceData.error || 'Wajah tidak dikenali')
        setLoading(false)
        setShowCamera(false)
        return
      }
      ...
      if (!approveRes.ok) {
        setError(approveData.error || 'Approval gagal')
        setLoading(false)
        setShowCamera(false)
        return
      }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-teal-50 to-white flex items-center justify-center px-4">
        <div className="card text-center max-w-sm">
          <p className="text-coral-600">❌ QR code tidak valid. Silakan scan ulang.</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-teal-50 to-white flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        ...
        <div className="card space-y-4">
          ...
          {error && (
            <div className="bg-coral-50 text-coral-600 text-sm px-3 py-2 rounded-lg">
              {error}
            </div>
          )}
